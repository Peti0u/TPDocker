import os, importlib, time, datetime, requests, pprint, shutil
...
MODELS_IMPORT     = 'home.models'
OUTPUT_GEN_MODELS = os.path.join('home', 'models.py' )
...
def model_dependencies( ModelName, Mapper):
    if 'linked_models' in Mapper and ModelName in Mapper['linked_models']:
...
# Helper
def generate_form(ModelName, ModelData, Mapper): 

    print( ' > GENERATE Form for [' + ModelName + '] ')

    aMap = {}
    aMap['__MODEL_NAME__'] = ModelName

    return process_tmpl( load_tmpl( 'model-form.tmpl' ), aMap ) 
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-f',                 type=str,            help='Input CSV File (input or remote)')
        ...
        parser.add_argument('-t',                 type=int,            help='Max Lines to load')
    ...
    def handle(self, *args, **kwargs):
        ...
        if info:    
            ...
            print(f"    -f                   : Input CSV File (input or remote)")
            ...
            print(f"    -t                   : Nbr of lines to load (default=99k)")
        ...
        print( ' > load_limit: ' + str( load_limit ) )      
        ...
        all_csv_files = list_files( INPUT_DIR, 'csv' )
        csv_files = []
        ...
        if input_file:
            ...
            if 'http' in input_file:
                ...
                tmp_file = h_random_ascii( 6 ) + '.csv'
                tmp_file_path = os.path.join( INPUT_DIR, tmp_file )
                if not file_save(tmp_file_path, r.text ):
                    print(f" > Error saving remove CSV file in MEDIA folder ["+input_file+"]")
                    return

                remote_model_name_v = input(" > Provide a name for your model: ")
                remote_model_name   = remote_model_name_v.replace(' ', '').title() + '.csv'
                model_path   = os.path.join(INPUT_DIR, remote_model_name)

                shutil.move( tmp_file_path, model_path )    

                csv_files = []
                csv_files.append( remote_model_name )
        ...
        ### Load Target Files: Models.py, Forms.py 
        MODEL_FILE_MARK = '### ### Below code is Generated ### ###'
        MODEL_FILE_c    = file_load( OUTPUT_GEN_MODELS )
        ...
        if simulation:
            ...
            for file_name in csv_files:
                ...
                if load or del_data: 
                    ...
                    # Check model exists and is migrated 

                    model_import = MODELS_IMPORT + '.' + model_name
                    model_class  = name_to_class( model_import )

                    if not model_class:
                        print( ' > ERR: Model [' + model_class + '] not found ' )
                        return 

                    #print(f" > Model Found: " +model_name+ " ")

                    if not check_model_migration( model_class ):
                        print( ' > ERR: Model [' + model_import + '] not MIGRATED ' )
                        return 

                    if del_data:
                        print( ' > DELETE data trigered for [' + model_import + ']') 
                    ...
                    for line in CVS_DATA:
                        ...
                        try:
                            ...
                            for val in fields:
                                ...
                                #print(' > ' + field_name + ' --> ' + str( val ) + ' --> ' + str ( field_val ) ) 
                ...
                # Model extraction case     
                    ...
                    #pprint.pp(model_data, indent=4)

                    # Generate Model      
                    model_django = os.linesep + generate_model( model_name, model_data, mapper )

                    #print( ' > model_django type(' +str( type( model_django ) )+ ') = ' + model_django ) 

                    if not MODEL_FILE_c:
                        print(f" > ERR loading OUTPUT file: " + OUTPUT_GEN_MODELS)
                        return     

                    # Update MODELS with generated Code    
                    MODEL_FILE_c += model_django
                    
                    if file_save( OUTPUT_GEN_MODELS, MODEL_FILE_c ):
                        print(f"   -> File saved")
                    else:
                        print(f"   -> ERR saving OUTPUT file: " + OUTPUT_GEN_MODELS)            
